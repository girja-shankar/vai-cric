...
export default function ScoringScreen({
  ...
  const [editTeams, setEditTeams] = useState(
    state.teams.map((t) => ({ ...t, players: [...t.players] })),
  );
  const [settingsTab, setSettingsTab] = useState<"match" | "team1" | "team2">(
    "match",
  );
  const [editCommonPlayer, setEditCommonPlayer] = useState<{
    enabled: boolean;
    name: string;
  }>({ enabled: false, name: "" });
  ...
  const openSettings = () => {
    ...
    setEditTeams(
      state.teams.map((t) => ({
        ...t,
        players: t.players
          .filter((p) => !p.id.startsWith("cp-"))
          .map((p) => ({ ...p })),
      })),
    );
    // Detect existing common player (id starts with "cp-")
    const existingCP = state.teams[0]?.players.find((p) =>
      p.id.startsWith("cp-"),
    );
    setEditCommonPlayer(
      existingCP
        ? { enabled: true, name: existingCP.name }
        : { enabled: false, name: "" },
    );
  ...
  const updateEditPlayer = (
    ...
    const newTeams = editTeams.map((t, ti) =>
      ti === teamIdx
        ? {
            ...t,
            players: t.players.map((p, pi) =>
              pi === playerIdx ? { ...p, name } : p,
            ),
          }
        : t,
    );
  ...
  const addEditPlayer = (teamIdx: number) => {
    const newTeams = editTeams.map((t, ti) =>
      ti === teamIdx
        ? {
            ...t,
            players: [...t.players, { id: `new-${Date.now()}`, name: "" }],
          }
        : t,
    );
    setEditTeams(newTeams);
  ...
  const removeEditPlayer = (teamIdx: number, playerIdx: number) => {
    ...
    if (
      innings.batters[player.id] &&
      (innings.batters[player.id].balls > 0 ||
        innings.batters[player.id].status !== "yetToBat")
    )
      return;
    if (innings.bowlers[player.id] && innings.bowlers[player.id].balls > 0)
      return;
  ...
  const handleSaveSettings = () => {
    ...
    if (editCommonPlayer.enabled && editCommonPlayer.name.trim()) {
      ...
      const cpBase = existingCP
        ? existingCP.id.replace(/-t[12]$/, "")
        : `cp-${Date.now()}`;
      cleanTeams[0].players.push({
        id: `${cpBase}-t1`,
        name: editCommonPlayer.name.trim(),
      });
      cleanTeams[1].players.push({
        id: `${cpBase}-t2`,
        name: editCommonPlayer.name.trim(),
      });
    }
    if (cleanTeams[0].players.length < 2 || cleanTeams[1].players.length < 2)
      return;
    dispatch({
  ...
  const [dismissalType, setDismissalType] = useState<
    "caught" | "bowled" | "runOut" | "stumped" | "other"
  >("caught");
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center bg-slate-50 gap-4">
        <p className="text-slate-500 text-sm">
          Invalid match state. Please go back and reselect players.
        </p>
        <button
  ...
  const nonStriker = innings.nonStrikerId
    ? innings.batters[innings.nonStrikerId]
    : null;
  ...
  const availableBatters = battingTeam.players.filter(
    (p) =>
      innings.batters[p.id].status === "yetToBat" ||
      innings.batters[p.id].status === "retiredOut",
  );
        ...
        <div className="flex-grow overflow-y-auto p-4 pb-24 space-y-2">
          {state.auditLog.length === 0 ? (
            ...
            [...state.auditLog].reverse().map((log, i) => (
              <div
                key={i}
                className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 text-sm text-slate-700"
              >
                {log}
              </div>
            ))
  ...
  return (
    <div className="h-full flex flex-col bg-slate-100 min-h-0 relative">
      {/* Top Bar */}
      <div className="bg-indigo-600 text-white px-4 pt-3 pb-5 shadow-lg relative z-10 shrink-0">
        <div className="relative flex items-center justify-between">
          ...
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-xl font-bold text-white/50 mx-0.5">-</span>
            <span className="text-2xl font-bold text-white/80">
              {innings.wickets}
            </span>
          </div>
          {/* Center: team name */}
          <h2 className="absolute left-1/2 -translate-x-1/2 text-white/80 text-sm font-bold tracking-widest uppercase">
            {battingTeam.name}
          </h2>
          {/* Right: overs */}
          <div className="bg-white/10 rounded-xl px-3 py-1.5">
            <div className="text-2xl font-black font-mono tracking-tight leading-none">
              {oversBowled.toFixed(1)}
              <span className="text-sm font-bold text-white/50">/{state.overs}</span>
            </div>
          </div>
        </div>

        {!isFirstInnings && state.innings1 && (
          <div className="flex justify-end items-center text-[10px] font-medium text-indigo-100 bg-indigo-700/50 rounded-lg p-2 backdrop-blur-sm">
...
<span className="font-bold text-base text-indigo-900">
              ...
              {striker.id === battingTeam.captainId && (
                <span className="text-[9px] font-bold text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded">C</span>
              )}
              <span className="text-[9px] font-bold text-indigo-600 bg-indigo-100 px-1.5 py-0.5 rounded uppercase tracking-wider">Strike</span>
            </div>
            <div className="font-mono flex items-baseline gap-1">
              <span className="font-black text-2xl text-indigo-900">{striker.runs}</span>
          ...
          {nonStriker ? (
            <div className="bg-white rounded-b-lg px-3 py-2.5 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                {nonStriker.id === battingTeam.captainId && (
                  <span className="text-[9px] font-bold text-amber-600 bg-amber-100 px-1 rounded">C</span>
                )}
              </div>
              <div className="font-mono flex items-baseline gap-1">
                <span className="font-bold text-lg text-slate-500">{nonStriker.runs}</span>
                <span className="text-slate-300 text-xs">({nonStriker.balls})</span>
              </div>
            </div>
          ) : (
        ...
        {/* Bowler */}
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-2 flex justify-between items-center shrink-0">
          <div>
            ...
            <div className="flex items-center gap-1.5 font-semibold text-sm text-slate-800">
              ...
              {bowler.id === bowlingTeam.captainId && (
          ...
          <div className="flex gap-3 font-mono text-center">
            <div>
              ...
              <div className="font-bold text-base">{bowler.runs}</div>
            </div>
            <div>
            ...
            {[0, 1, 2, 3].map((runs) => {
              ...
              return (
                <button
                  ...
                  className={`py-3.5 rounded-xl font-bold text-xl transition-all active:scale-90 flex items-center justify-center relative overflow-hidden bg-white text-slate-700 border-2 border-indigo-200 shadow-sm ${isFlashing ? "ring-4 ring-indigo-400 scale-95" : ""}`}
                >
                  {isFlashing && (
                    <span className="absolute inset-0 bg-indigo-400/20 animate-ping rounded-xl" />
                  )}
                  {runs}
                </button>
              );
            ...
            {[4, 6].map((runs) => {
              ...
              return (
                <button
                  key={runs}
                  onClick={() => handleDelivery(runs)}
                  className={`py-2.5 rounded-xl font-bold text-2xl transition-all active:scale-90 flex items-center justify-center relative overflow-hidden bg-emerald-50 text-emerald-700 border-2 border-emerald-400 shadow-sm ${isFlashing ? "ring-4 ring-emerald-400 scale-95" : ""}`}
                >
                  {isFlashing && (
                    <span className="absolute inset-0 bg-emerald-400/20 animate-ping rounded-xl" />
                  )}
                  {runs}
                </button>
              );
        ...
        <div className="grid grid-cols-2 gap-2 shrink-0">
          <button
            ...
            className={`py-3.5 flex items-center justify-center rounded-xl font-bold text-xl text-slate-700 bg-white border-2 border-indigo-200 shadow-sm active:scale-90 relative overflow-hidden ${flashId === "wide-0" ? "ring-4 ring-amber-400 scale-95" : ""}`}
          >
            {flashId === "wide-0" && (
              <span className="absolute inset-0 bg-amber-400/20 animate-ping rounded-xl" />
            )}
            Wide
          </button>
          <button
      ...
      {/* Bottom Bar */}
      <div className="bg-white border-t border-slate-100 px-4 py-2 flex justify-around items-center shrink-0">
        <button
          onClick={() => setShowEndInningsModal(true)}
          className="flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-xl text-rose-500 hover:text-rose-700 hover:bg-rose-50 transition-colors"
        >
        ...
        <button
          ...
          className="flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-xl text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-colors"
        >
        ...
        <button
          ...
          className="flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-xl text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-colors disabled:opacity-30"
        >
          ...
          <div className="bg-white w-full max-w-sm rounded-2xl p-4 shadow-2xl animate-in slide-in-from-bottom-8">
            ...
            <div className="space-y-3">
              <div>
                ...
                <div
                  className={`grid gap-1.5 ${nonStriker ? "grid-cols-2" : "grid-cols-1"}`}
                >
                  <button
                  ...
                  {nonStriker && (
                    ...
                    >
                      {
                        battingTeam.players.find((p) => p.id === nonStriker.id)
                          ?.name
                      }
                    </button>
                  )}
              ...
              {dismissalType === "runOut" && (
                <div>
                  ...
                  <p className="text-[9px] text-slate-400 mt-0.5">
                    Runs before dismissal (e.g. run out on 2nd run)
                  </p>
                </div>
              )}
              ...
              {outType !== "retiredOut" &&
                (() => {
                  const survivorId =
                    outBatsmanId === innings.strikerId
                      ? innings.nonStrikerId
                      : innings.strikerId;
                  const incomingId = nextBatsmanId || null;
                  const ids = [survivorId, incomingId].filter(
                    (id): id is string => !!id,
                  );
                  if (ids.length < 2) return null;
                  const autoNext = (() => {
                    const rotate = wicketRuns % 2 !== 0;
                    if (outBatsmanId === innings.strikerId)
                      return rotate
                        ? (innings.nonStrikerId ?? "")
                        : (incomingId ?? "");
                    return rotate
                  ...
                  return (
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        {ids.map((id) => {
                          const name =
                            battingTeam.players.find((p) => p.id === id)
                              ?.name || "Unknown";
            ...
            <div className="mt-4 flex gap-2">
              ...
              <button
                ...
                disabled={
                  !outBatsmanId ||
                  (!nextBatsmanId && availableBatters.length > 0)
                }
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm font-mono text-slate-500">
                      {bStats
                        ? `${(Math.floor(bStats.balls / 6) + (bStats.balls % 6) / 10).toFixed(1)} O`
                        : "0.0 O"}
                    </span>
                  </button>
                );
          ...
          <div className="bg-white flex flex-col h-full animate-in slide-in-from-bottom-8">
                ...
                <div>
                  ...
                  {innings.balls > 0 && editOvers * 6 < innings.balls && (
                    <p className="text-xs text-rose-500 mt-2">
                      Overs cannot be less than already bowled (
                      {(innings.balls / 6).toFixed(1)} overs).
                    </p>
                  )}
                  ...
                  <div className="bg-amber-50 rounded-xl p-4 border border-amber-100 mt-5">
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <span
                          className={`absolute top-0.5 left-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform ${editCommonPlayer.enabled ? "translate-x-5" : ""}`}
                        />
                      </button>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-slate-400">
                          {team.players.filter((p) => p.name.trim()).length}{" "}
                          players
                        </span>
                      </div>
                      <div className="space-y-2">
                        {team.players.map((p, i) => {
                          ...
                          const hasStats =
                            (innings.batters[p.id] &&
                              (innings.batters[p.id].balls > 0 ||
                                innings.batters[p.id].status !== "yetToBat")) ||
                            (innings.bowlers[p.id] &&
                              innings.bowlers[p.id].balls > 0);
                          return (
                            <div key={p.id} className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                value={p.name}
                                onChange={(e) =>
                                  updateEditPlayer(teamIdx, i, e.target.value)
                                }
            ...
            <div className="p-4 bg-white border-t border-slate-100 shrink-0">
              <button
                ...
                disabled={
                  editOvers * 6 < innings.balls ||
                  editTeams.some(
                    (t) => t.players.filter((p) => p.name.trim()).length < 2,
                  )
                }
